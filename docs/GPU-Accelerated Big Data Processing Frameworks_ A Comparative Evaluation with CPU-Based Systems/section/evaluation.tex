...
Phần này đánh giá đúng theo notebook gốc của nhóm, tức là bài toán survival analysis với metric chính là C-index (concordance index), không phải ROC AUC. Notebook \texttt{predictcare-new_readmission.ipynb} dùng \texttt{CoxPHFitter} và \texttt{XGBSEStackedWeibull}, sau đó báo cáo C-index trên train/validation/test.
...
C-index (XGBSE) & 0.8087 & 0.7860 & 0.7813 \\
Best Iteration & \multicolumn{3}{c|}{189 (early stop on val)} \\
\hline
\end{tabular}
\caption{XGBSE Survival Model Performance (Ray MIMIC Notebook Baseline)}
\label{table:xgbse_metrics}
\end{table}

Notebook cũng cho baseline Cox PH với C-index trên train/validation lần lượt là 0.7225 và 0.7234, sau đó mô hình XGBSE cải thiện lên 0.7813 trên test. Đây là metric phù hợp với specification của bài thực hành: tối ưu C-index trên test set và hướng tới C-index \geq 0.70.

\begin{itemize}
    \item \textbf{Test C-index = 0.7813:} Mô hình đạt hiệu suất tốt trên tập test không nhìn thấy, cho thấy feature pipeline từ Ray/Gold dataset đủ mạnh cho survival analysis.
    \item \textbf{Generalization gap (Train → Test):} 0.8087 → 0.7813 = 0.0274 (2.74\%), cho thấy mô hình tổng quát hóa khá tốt.
