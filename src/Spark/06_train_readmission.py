"""
06_train_readmission.py — PySpark Pipeline Step 6
Train an XGBoost readmission risk model and evaluate it with C-index.

PySpark equivalent of Ray step 6.
"""
...
# Try reading pandas parquet first, then fallback to Spark
gold_path = f"{OUT}/gold_dataset_spark.parquet"
try:
    gold_df = pd.read_parquet(gold_path)
except FileNotFoundError:
    # Fallback to Spark parquet directory
...
model_path = str(OUT / "xgb_readmission_model_spark.json")
model.save_model(model_path)
print(f"[INFO] Model saved to: {model_path}")

metrics = {
    "task": "30-day readmission (PySpark)",
...
metrics_path = str(LOGS_DIR / "readmission_metrics_spark.json")
